...
tune.maxent <- function(feature_matrix, code_vector, nfold=3, showall=FALSE, verbose=FALSE) {

    feature_matrix <- as.compressed.matrix(feature_matrix)
    if (verbose) messgae("Testing 18 parameter configurations...\n")
    ...
    for (n in 1:length(l1_params)) {
        ...
        if (verbose) cat(paste("Configuration: ",n,"\tAccuracy (",nfold,"-fold cross-validation): ",mean(cv_accuracy),"\n",sep=""))
    ...
    colnames(optimal) <- c("l1_regularizer","l2_regularizer","use_sgd","set_heldout","accuracy","pct_best_fit")
